...
import { getIsCredentialRecoveryFlow, getUserEnteredEmailOrUsername } from '@de-care/de-care-use-cases/account/state-account-credentials-management';
            ...
            map(([transactionStateExists, isCredentialRecoveryFlow]) => {
                if (!transactionStateExists) {
                    if (isCredentialRecoveryFlow) {
                        ...
                        this._navigateTo('/account/credentials/forgot-password');
                    }
                    return true;
                }
