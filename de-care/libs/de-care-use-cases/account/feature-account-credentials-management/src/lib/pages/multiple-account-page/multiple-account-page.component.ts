...
@Component({
    selector: 'de-care-multiple-account-page',
    templateUrl: './multiple-account-page.component.html',
    styleUrls: ['./multiple-account-page.component.scss'],
})
export class MultipleAccountPageComponent implements OnInit, ComponentWithLocale, OnDestroy {
        ...
        if (account) {
            ...
            this._verifyAccountWorkflowService.build(payLoad).subscribe({
                next: (response: VerifyAccountWorkflowServiceResult) => {
                    ...
                    this._navigateTo(response === 'USE_BOTH' || response === 'USE_EMAIL' || response === 'USE_PHONE' ? '../verify-your-account' : '/account/registration');
                },
            });
        } else {
