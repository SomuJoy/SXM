...
import {
    getSelectedAccount,
    RecoverPasswordFromEmailWorkflowServices,
    RecoverPasswordFromPhoneWorkflowServices,
...
@Component({
    selector: 'de-care-verify-your-account',
    templateUrl: './verify-your-account.component.html',
    styleUrls: ['./verify-your-account.component.scss'],
})
export class VerifyYourAccountComponent implements OnInit, ComponentWithLocale, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        private _recoverPasswordFromEmailWorkflowServices: RecoverPasswordFromEmailWorkflowServices,
        private _recoverPasswordFromPhoneWorkflowServices: RecoverPasswordFromPhoneWorkflowServices,
        ...
        this._store.select(getQueryParams).subscribe((val) => {
            if (val) {
                this.allQueryParam = convertObjectToUrlQueryParamsString(val);
                if (this.allQueryParam.length > 0) {
                    this.allQueryParam = '&' + this.allQueryParam;
                }
            } else {
                this.allQueryParam = '';
            }
    ...
    onVerifyAccountClick(data) {
        ...
        this._store.dispatch(setOrganicAccountType({ organicAccountType: data?.accountData?.accountType }));
        ...
        if (data?.formValue?.verifyType === 'text') {
            ...
            this._recoverPasswordFromPhoneWorkflowServices.build(param).subscribe({
                ...
                error: (error) => {
                    if (error?.error?.error?.errorCode === 'ERROR_IN_SENDING_TEXT' || error?.error?.error?.errorCode === 'INVALID_USER_ENTERED_PHONE_NUMBER') {
                        ...
                        if (error?.error?.error?.errorCode === 'INVALID_USER_ENTERED_PHONE_NUMBER') {
                            this.verifyAccountFormComponentApi.showPhoneNumberNotMatchedError(true);
                        } else {
            ...
            if (this.allQueryParam.length > 1) {
                this.allQueryParam = this.allQueryParam.slice(1, this.allQueryParam.length);
            }
            const param = {
                setResetKey: true,
                subscriptionId: data?.accountData?.selectedSubscriptionId ? data?.accountData?.selectedSubscriptionId : data?.accountData?.subscriptions[0]?.id,
