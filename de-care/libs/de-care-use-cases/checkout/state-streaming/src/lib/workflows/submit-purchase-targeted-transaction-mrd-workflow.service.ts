...
import { getPayloadForMrdPurchaseTargetedTransaction } from '../state/selectors';
...
export type SubmitPurchaseTargetedTransactionMrdWorkflowErrors = 'CREDIT_CARD_FAILURE' | 'SYSTEM';

@Injectable({ providedIn: 'root' })
export class SubmitPurchaseTargetedTransactionMrdWorkflowService implements DataWorkflow<void, boolean> {
    ...
    build(): Observable<boolean> {
        return this._store.select(getPayloadForMrdPurchaseTargetedTransaction).pipe(
            take(1),
            concatMap((request) =>
                this._addSubscriptionWorkflowService.build({ ...request, subscriptionId: undefined }, false).pipe(
            ...
            catchError((error) => {
                if (error instanceof CreditCardUnexpectedError) {
                    ...
                    return throwError('CREDIT_CARD_FAILURE' as SubmitPurchaseTargetedTransactionMrdWorkflowErrors);
                }
                return throwError('SYSTEM' as SubmitPurchaseTargetedTransactionMrdWorkflowErrors);
            })
        );
    }
}
