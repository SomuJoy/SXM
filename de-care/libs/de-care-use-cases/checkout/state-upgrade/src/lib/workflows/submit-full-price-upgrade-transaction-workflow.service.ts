...
export type SubmitFullPriceUpgradeTransactionWorkflowServiceError = 'SYSTEM' | 'CREDIT_CARD_FAILURE';

@Injectable({ providedIn: 'root' })
export class SubmitFullPriceUpgradeTransactionWorkflowService implements DataWorkflow<void, boolean> {
    constructor(
        ...
        return combineLatest([this._store.select(getPayloadForPurchaseTransaction), this._store.select(getSelectedRadioIsClosed)]).pipe(
            ...
            catchError((error) => {
                if (error instanceof CreditCardUnexpectedError) {
                    ...
                    return throwError('CREDIT_CARD_FAILURE' as SubmitFullPriceUpgradeTransactionWorkflowServiceError);
                }
                return throwError('SYSTEM' as SubmitFullPriceUpgradeTransactionWorkflowServiceError);
            })
        );
    }
}
