...
import { Component, ChangeDetectionStrategy, OnInit, AfterViewInit, ViewChild, Inject, NgModule } from '@angular/core';
...
import {
    ...
    loadOrganicPurchaseDataIfNotAlreadyLoadedAsync,
...
@Component({
    selector: 'step-organic-credentials-page',
...
export class StepOrganicCredentialsPageComponent implements ComponentWithLocale, OnInit, AfterViewInit {
    ...
    activeSubscriptionsViewModel$ = this._store.select(getActiveStreamingSubscriptionViewModel);
    ...
    ngAfterViewInit(): void {
        const locationState = this._location?.getState() as any;
        ...
        if (locationState?.passwordError) {
            this._passwordFormControl?.setErrors({ generic: true });
            this._passwordFormControl?.markAsTouched();
        }
        if (locationState?.passwordHasPiiDataError) {
            this._passwordFormControl?.setErrors({ piiData: true });
            this._passwordFormControl?.markAsTouched();
        }
        this._store
            .select(getProspectTokenData)
            .pipe(
                take(1),
                filter((data) => !!data?.userName)
            )
            .subscribe(({ userName }) => {
                this.accountLookupForm.get('email').setValue(userName);
            });
    }
...
@NgModule({
    declarations: [StepOrganicCredentialsPageComponent],
    exports: [StepOrganicCredentialsPageComponent],
    imports: [
        CommonModule,
        ReactiveFormsModule,
        TranslateModule,
...
export class StepOrganicCredentialsPageComponentModule {}
