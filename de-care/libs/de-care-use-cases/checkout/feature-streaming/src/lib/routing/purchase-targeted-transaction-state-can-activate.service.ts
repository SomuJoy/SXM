...
import { targetedTransactionExists } from '@de-care/de-care-use-cases/checkout/state-streaming';
...
export class PurchaseTargetedTransactionStateCanActivateService implements CanActivate {
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<any> {
        return this._store.select(targetedTransactionExists).pipe(
            take(1),
            map((targetedTransactionExists) => {
                if (!targetedTransactionExists) {
                ...
                return targetedTransactionExists;
            })
        );
    }
}
