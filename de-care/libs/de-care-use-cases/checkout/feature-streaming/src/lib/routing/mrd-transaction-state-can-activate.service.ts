...
import { mrdTransactionStateExists } from '@de-care/de-care-use-cases/checkout/state-streaming';
...
export class MrdTransactionStateCanActivateService implements CanActivate {
    constructor(private _router: Router, private _store: Store) {}

    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<any> {
        return this._store.select(mrdTransactionStateExists).pipe(
            take(1),
            map((mrdTransactionStateExists) => {
                if (!mrdTransactionStateExists) {
                ...
                return mrdTransactionStateExists;
            })
        );
    }
}
