...
import { CustomerValidationAddressesWorkFlowService, CustomerValidation } from '@de-care/domains/customer/state-customer-verification';
...
export class PaymentInfoComponent implements OnInit, OnDestroy, AfterViewChecked, OnChanges, AfterViewInit {
    @Input() userClickedSubscribe: boolean;
    @Input() isClosedRadio: boolean;
    paymentInfoBillingAddressModalAriaDescribedbyTextId = uuid();
    paymentInfoServiceAddressModalAriaDescribedbyTextId = uuid();
    //================================================
    //===            Lifecycle events              ===
    //================================================
    constructor(
    ...
    private _continueValidPaymentForm() {
        this.loading = true;
        this.invalidZipFromLookup$ = new BehaviorSubject(false);
        ...
        if (this.creditCardInfo) {
            ...
            if ((!this.f?.email || !this.f?.email?.errors) && this.selectedCC && this.creditCardInfo.status === 'ACTIVE') {
