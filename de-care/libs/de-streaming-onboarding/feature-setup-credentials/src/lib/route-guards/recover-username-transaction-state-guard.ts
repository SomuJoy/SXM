...
import { getIsCredentialRecoveryFlow, getUserEnteredEmailAndLastname } from '@de-care/de-streaming-onboarding/state-setup-credentials';
            ...
            map(([transactionStateExists, isCredentialRecoveryFlow]) => {
                if (!transactionStateExists) {
                    if (isCredentialRecoveryFlow) {
                        ...
                        this._navigateTo('/setup-credentials/forgot-username');
                    }
                    return true;
                }
