...
@Component({
    selector: 'de-streaming-account-not-found',
    templateUrl: './account-not-found.component.html',
    styleUrls: ['./account-not-found.component.scss'],
})
export class AccountNotFoundComponent implements OnInit, AfterViewInit, ComponentWithLocale, OnDestroy {
            ...
            this._loadAccountFromCredentialRecoveryLookupWorkflowService.build(param).subscribe({
                ...
                error: (error) => {
                    if (error?.error?.error?.errorType === 'SYSTEM') {
                        ...
                        if (param.source === 'oac' && error?.error?.error?.errorCode === 'CUSTOMER_TYPE_NOT_SUPPORTED') {
