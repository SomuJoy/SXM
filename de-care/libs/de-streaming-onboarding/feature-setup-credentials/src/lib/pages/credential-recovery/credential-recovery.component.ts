...
import { getSrcQueryParam, setIsCredentialRecoveryFlow } from '@de-care/de-streaming-onboarding/state-setup-credentials';
import { take } from 'rxjs/operators';

@Component({
    selector: 'de-streaming-onboarding-credential-recovery',
    templateUrl: './credential-recovery.component.html',
    styleUrls: ['./credential-recovery.component.scss'],
})
export class CredentialRecoveryComponent implements OnInit, AfterViewInit {
    ...
    onRecoveryCredentialOptionSelected(deviceIdSelection: RecoveryOptionSelection) {
        this._navigateTo(deviceIdSelection?.recoveryOption === 'password' ? '../forgot-password' : '../forgot-username');
    }
