...
const routeUrlBase = 'http://localhost:4201/subscribe/checkout/purchase/streaming/organic';
...
Given(/^a customer visits the page with the program code CATMSRTP3MOFREE$/, () => {
    cy.visit(`${routeUrlBase}?programcode=CATMSRTP3MOFREE`);
});
...
Given('a customer goes through the organic streaming purchase steps starting on credentials interstitial with the program code CATMSRTP3MOFREE', () => {
    cy.visit(`${routeUrlBase}/creds-int?programcode=CATMSRTP3MOFREE`);
    cy.get('#mainContent a[sxmUiDataClickTrack="routing"]').click({ force: true });
    completeTransactionFromCredentialsStep();
});

Given('a customer goes through the organic streaming purchase steps starting on credentials with the program code CATMSRTP3MOFREE', () => {
    cy.visit(`${routeUrlBase}/creds?programcode=CATMSRTP3MOFREE`);
    completeTransactionFromCredentialsStep();
});
