...
import { stubOffersCustomerCancelStreamingWithMusicShowcase } from '../../../../support/stubs/de-microservices/offers';
...
Given(/^the Music Showcase plan is returend by PEGA$/, () => {
    stubOffersCustomerCancelStreamingWithMusicShowcase();
});
...
Then(/^the experience should load the special preselected offer page$/, () => {
    cy.get('[data-test="PreSelectedPlanOfferHeader"]').should('exist');
});
