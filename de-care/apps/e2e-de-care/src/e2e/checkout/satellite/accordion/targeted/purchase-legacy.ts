...
import {
    stubOffersUpsellInfSatelliteTargetedSelfPayPromoPackageAndTerm,
    stubOffersUpsellSatelliteTargetedSelfPayPromoPackageAndTerm,
...
When(/^a customer visits the satellite accordion targeted flow for a self pay promo with a valid radio id and account number$/, () => {
    ...
    cy.visit(`/subscribe/checkout?RadioID=CNT0008V&programcode=6FOR30SELECT&act=10000207275`);
    cy.wait(['@offersCall']);
});
...
When(/^a customer visits the satellite accordion targeted flow for a self pay promo with a valid radio id and account number and upcode$/, () => {
    ...
    stubOffersUpsellSatelliteTargetedSelfPayPromoPackageAndTerm();
    stubOffersUpsellInfSatelliteTargetedSelfPayPromoPackageAndTerm();
    cy.visit(`/subscribe/checkout?RadioID=CNT0008V&programcode=6FOR50AA&upcode=6FOR50AA&act=10000207275`);
    cy.wait(['@offersCall']);
});
